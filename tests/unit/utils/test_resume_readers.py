...
def test_read_resume_any_txt_routes_to_text(tmp_path):
    p = tmp_path / "resume.txt"
    p.write_text("Text resume", encoding="utf-8")
    assert read_resume_any(str(p)) == "Text resume"


def test_read_resume_any_md_routes_to_text(tmp_path):
    p = tmp_path / "resume.md"
    p.write_text("# Title\nBody", encoding="utf-8")
    assert read_resume_any(str(p)) == "# Title\nBody"


def test_read_resume_any_docx_routes_to_docx(tmp_path):
    p = tmp_path / "resume.docx"
    doc = Document()
